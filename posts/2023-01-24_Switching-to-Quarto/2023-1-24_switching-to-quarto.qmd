...
1.  Assuming you **Create New Project** in RStudio a subdirectory will be created called `posts`. Create a subfolder in your `posts` folder to hold converted file. Use any name but I like the form ***yyyy-mm-dd_name-of-post***. This will allow displaying the posts in date order.
...
7.  Most of your posts will have images you added or images that were generated during the render. We need to put those old images in a place where Quarto can find them. Create subfolder below the one just created called `img`.
...
Bonus: I really liked the Blogdown add-in for R Studio as it made initiating a new post very easy. Thomas Mock has made a start on this with a [quick function to start a new Quarto post](https://themockup.blog/posts/2022-11-08-use-r-to-generate-a-quarto-blogpost/). Check it out.
